...
void CSevenSegmentDisplay::activateNextDigit() {
	...
	if(m_values[(m_activeDigit + 1) % 4]<0){
		turnDisplayOff();
	}
...
int& CSevenSegmentDisplay::operator[](int position) {
...
void CSevenSegmentDisplay::setValue(float value) {
	...
	// Falls Eingabe negativ ist...
	else if (value < 0.0) {

		// Anzeigestelle "n" wird ausgeschaltet.
		turnDisplayOff();
	}
