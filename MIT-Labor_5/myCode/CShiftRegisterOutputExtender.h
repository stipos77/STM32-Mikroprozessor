...
private:
	// Zeiger auf DigitalOut Objekt (SER/ Dateneingang).
	DigitalOut *m_serialData;

	// Zeiger auf DigitalOut Objekt (SRC/ Datenausgang).
	DigitalOut *m_shiftRegisterClock;

	// Zeiger auf DigitalOut Objekt (CLK).
	DigitalOut *m_bufferRegisterClock;

	// Vorbereiteten auszugebenden Wert.
...
public:
	...
	/**
	 * Schieben Sie den übergebenen Wert durch Ansteuern der BSRR-Register für
	 * den Ausgang, der das Taktsignal liefert und den Ausgang, der das
	 * Datensignal liefert in einer Schleife (!) in das Schieberegister.
	 */
	void doShift(GPIO_TypeDef *clockPort,
